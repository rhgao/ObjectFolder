...
class MultiNetwork(nn.Module):
    def __init__(self, num_networks, num_position_channels, num_direction_channels, num_light_channels, num_output_channels, hidden_layer_size, num_hidden_layers, refeed_position_index=None, late_feed_direction=False,
        ...
        self.num_light_channels = num_light_channels
        ...
        if self.late_feed_direction:
            ...
            self.direction_layer = linear_layer(self.num_direction_channels + self.num_light_channels + self.hidden_layer_size, self.direction_layer_size, self.nonlinearity, self.use_hard_parameter_sharing_for_color)
    ...
    def forward(self, x, batch_size_per_network=None, random_directions=None):
        if self.late_feed_direction:
            if isinstance(x, list):
                positions, directions, lights = x
                # frees memory of inputs
                x[0] = None 
                x[1] = None
                x[2] = None
            else:
                positions, directions, lights  = torch.split(x, [self.num_position_channels, self.num_direction_channels, self.num_light_channels], dim=-1)
            h = positions
            ...
            h = torch.cat([feature, directions, lights], -1)
    ...
    def extract_single_network(self, network_index):
        single_network = MultiNetwork(1, self.num_position_channels, self.num_direction_channels, self.num_light_channels, self.num_output_channels,
...
def query_multi_network(multi_network, domain_mins, domain_maxs, points, directions, lights,
    ...
    lights_flat = lights.unsqueeze(1).expand(points.size()).reshape(-1, 3)
    points_reordered = points_flat[active_samples_mask] 
    directions_reordered = directions_flat[active_samples_mask]
    lights_reordered = lights_flat[active_samples_mask]
    ...
    lights_reordered = lights_reordered[reorder_indices]
    ...
    if light_fourier_embedding is not None:
        embedded_lights = light_fourier_embedding(lights_reordered.unsqueeze(0), implementation=fourier_embedding_implementation).squeeze(0)
    else:
        embedded_lights = lights_reordered
    embedded_points_and_dirs = [embedded_points, embedded_dirs, embedded_lights]
    del embedded_points
    del embedded_dirs
...
def build_multi_network_from_single_networks(single_networks, transpose_weight = True, linear_implementation = 'multimatmul', view_dependent_dropout_probability=-1):
    ...
    multi_network = MultiNetwork(num_networks, p.num_position_channels, p.num_direction_channels, p.num_light_channels, p.num_output_channels, p.hidden_layer_size, p.num_hidden_layers, p.refeed_position_index, p.late_feed_direction,
