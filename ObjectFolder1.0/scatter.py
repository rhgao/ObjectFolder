...
def scatter_results(intersect, indices, N_rays, keys, N_samples, N_importance=None):
    ...
    for k in keys:
        if k == 'z_vals':
            ...
            tensor = torch.full((N_rays, N_samples, 4), 1000.0, dtype=torch.float)  # [R, S, 4]
        elif k == 'pts':
            tensor = torch.full((N_rays, N_samples, 3), 1000.0, dtype=torch.float)  # [R, S, 3]
        elif 'rgb' in k:
            tensor = torch.zeros((N_rays, N_sampels, 3), dtype=torch.float)  # [R, S, 3]
        elif 'alpha' in k:
            tensor = torch.zeros((N_rays, N_samples, 1), dtype=torch.float)  # [R, S, 1]
        else:
...
def scatter_coarse_and_fine(
    ...
    ret = scatter_results(
        ...
        indices=indices,
        N_rays=num_rays,
        key=['z_vals', 'rgb', 'alpha', 'raw', 'z_samples'],
