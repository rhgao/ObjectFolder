...
def create_multi_network(num_networks, num_position_channels, num_direction_channels, num_light_channels, num_output_channels, linear_implementation, cfg):
    ...
    return MultiNetwork(num_networks, num_position_channels, num_direction_channels, num_light_channels, num_output_channels,
...
def preprocess_examples(batch_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding):
    if cfg['outputs'] == 'density':
        ...
        batch_lights = batch_examples[:, :, 6:9]
        if cfg['num_frequencies_light'] > 0:
            batch_lights = light_fourier_embedding(batch_lights)
        batch_inputs = torch.cat((batch_positions, batch_directions, batch_lights), dim=2)
        batch_targets = batch_examples[:, :, 9:13]
...
def train_and_test_multi_network(multi_network, all_examples, domain_mins, domain_maxs, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding, processing_saturated_nodes, cfg):
    ...
    for iteration in trange(1, cfg['iterations'] + 1):
        ...
        train_batch_inputs, train_batch_targets = preprocess_examples(train_examples[:, indices].to(domain_mins), domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding)
        ...
        if iteration % cfg['test_every'] == 0 or iteration == cfg['iterations']:
            errors_per_point, errors_per_network, errors_per_network_color, errors_per_network_density, saturation =\
                test_multi_network(multi_network, test_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding)
...
def test_multi_network(multi_network, test_examples, domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding):
    ...
    with torch.no_grad():
        ...
        while start < num_test_examples:
            ...
            test_batch_inputs, test_batch_targets = preprocess_examples(test_examples[:, start:end].to(domain_mins), domain_mins, domain_maxs, cfg, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding)
...
def train_and_test_nodes(node_batch, pretrained_nerf, processing_saturated_nodes, cfg, dev):
    ...
    if cfg['outputs'] == 'color_and_density':
        ...
        light_num_input_channels, light_fourier_embedding = create_multi_network_fourier_embedding(num_networks, cfg['num_frequencies_light'])
        num_output_channels = 4
    MultiNetworkLinear.rng_state = None
    multi_network =  create_multi_network(num_networks, position_num_input_channels, direction_num_input_channels, light_num_input_channels, 4, 'bmm', cfg).to(dev)
    all_examples = torch.empty(num_networks * cfg['num_examples_per_network'], 9 + num_output_channels) # x,y,z,dir_x,dir_y,dir_z,light_x,light_y,light_z,r,g,b,a
    start = 0
    for network_index in range(num_networks):
        ...
        all_examples[start:end, 6:9] = torch.tensor(get_random_lights(cfg['num_examples_per_network']), dtype=torch.float)
    points_and_dirs = all_examples[:, 0:9]
    ...
    with torch.no_grad():
        ...
        while start < num_points_and_dirs:
            ...
            if cfg['outputs'] == 'color_and_density':
                all_examples[start:end, 9:12] = 1.2 * (F.sigmoid(raw_output[:, 0:3]).cpu() - 0.5) + 0.5 # Apply sigmoid to color outputs
                # all_examples[start:end, 9:12] = F.sigmoid(raw_output[:, 0:3]).cpu()# Apply sigmoid to color outputs
                all_examples[start:end, 12] = process_density(raw_output)
            if has_flag(cfg, 'use_premultiplied_colors'):
                all_examples[start:end, 9:12] *= all_examples[start:end, 12]
            del raw_output
            start = end
    if cfg['outputs'] == 'density':
    ...
    test_points, errors_per_point, best_errors_per_network, best_errors_per_network_color, best_errors_per_network_density, saturation =\
        train_and_test_multi_network(multi_network, all_examples, domain_mins, domain_maxs, position_fourier_embedding, direction_fourier_embedding, light_fourier_embedding, processing_saturated_nodes, cfg)
